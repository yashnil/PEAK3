...
def test_shaquille_oneal_cannot_play_small_forward():
    ...
    rights = career_rights("shaquille-o-neal")
    assert P.canonical_positions("shaquille-o-neal") == frozenset({"C"})
    assert not P.is_legal("shaquille-o-neal", "SF", rights)
    assert not P.is_legal("shaquille-o-neal", "PF", rights)
    assert P.is_legal("shaquille-o-neal", "C", rights)
...
def test_lebron_james_can_play_point_guard():
    ...
    rights = career_rights("lebron-james")
    assert "PG" in P.canonical_positions("lebron-james")
    assert P.is_legal("lebron-james", "PG", rights)
    assert P.legal_slots("lebron-james", rights) == frozenset(
...
def test_an_identity_with_no_position_data_is_illegal_everywhere():
    ...
    rights = career_rights("not-a-real-player")
    assert P.canonical_positions("not-a-real-player") == frozenset()
    assert P.legal_slots("not-a-real-player", rights) == frozenset()
    for slot in SLOT_TYPES:
        assert not P.is_legal("not-a-real-player", slot, rights)


def test_bench_accepts_any_player_with_a_recognized_position():
    rights = career_rights("shaquille-o-neal", "lebron-james", "john-stockton")
    for slug in ("shaquille-o-neal", "lebron-james", "john-stockton"):
        for bench_slot in BENCH_SLOT_TYPES:
            assert P.is_legal(slug, bench_slot, rights)
...
def test_every_eligible_identity_has_at_least_one_legal_slot(index):
    ...
    Asserted at the SEASON grain -- the grain the draft enforces -- and for
    every roll separately, because a card is what a pick is made against. The
    career-grain form of this test passed while individual cards could still
    have been empty."""
    for franchise_id, decade in index.rolls():
        for slug in index.eligible_slugs(franchise_id, decade):
            assert index.card_slot_rights(slug, franchise_id, decade), (
                f"{slug} has no legal slot on {franchise_id} x {decade}"
            )
...
def test_validate_roster_rejects_an_illegal_placement():
    ...
    check = P.validate_roster(roster, career_rights("shaquille-o-neal"))
    assert not check.ok
...
def test_validate_roster_rejects_the_same_identity_twice():
    ...
    check = P.validate_roster(roster, career_rights("lebron-james"))
    assert not check.ok
...
def test_validate_roster_accepts_a_partial_roster():
    ...
    assert P.validate_roster(roster, career_rights("shaquille-o-neal")).ok
...
def test_reposition_that_would_be_illegal_raises_and_changes_nothing():
    ...
    with pytest.raises(P.IllegalPlacement) as excinfo:
        P.apply_reposition(roster, "C", "SF", career_rights("shaquille-o-neal", "lebron-james"))
...
def test_reposition_returns_a_new_legal_roster():
    ...
    rights = career_rights("lebron-james", "larry-bird")
    moved = P.apply_reposition(roster, "PF", "SF", rights)
    assert moved["PF"] == "larry-bird"
    assert moved["SF"] == "lebron-james"
    assert P.validate_roster(moved, rights).ok
...
def test_reposition_is_refused_when_only_one_direction_is_legal():
    ...
    roster["SF"] = "larry-bird"
    with pytest.raises(P.IllegalPlacement) as excinfo:
        P.apply_reposition(roster, "PG", "SF", career_rights("lebron-james", "larry-bird"))
    assert excinfo.value.code == "illegal_slot"
...
def test_reposition_into_an_empty_slot_is_a_move():
    roster = P.empty_roster()
    roster["SG"] = "lebron-james"
    moved = P.apply_reposition(roster, "SG", "PG", career_rights("lebron-james"))
...
def test_three_centres_cannot_fill_three_different_positions():
    ...
    rights = career_rights(*centres, "larry-bird", "tim-duncan", "scottie-pippen")
    assert not P.can_complete_roster(roster, centres, rights)
    # Only the C slot is actually reachable from that pool.
    assert P.legal_players_for_slot("C", centres, rights) == tuple(centres)
    assert P.legal_players_for_slot("PG", centres, rights) == ()
...
def test_can_complete_roster_succeeds_with_a_genuinely_capable_pool():
    ...
    rights = career_rights(*pool)
    assert P.can_complete_roster(roster, pool, rights)

    witness = P.completion_witness(roster, pool, rights)
    ...
    for slot, slug in witness.items():
        assert P.is_legal(slug, slot, rights)
...
def test_can_complete_roster_counts_the_bench_slot(index):
    ...
    rights = career_rights(*five, "scottie-pippen")
    assert not P.can_complete_roster(P.empty_roster(), five, rights)
    assert P.can_complete_roster(P.empty_roster(), five + ["scottie-pippen"], rights)
    assert STARTER_SLOT_TYPES == ("PG", "SG", "SF", "PF", "C")


def test_completion_witness_is_none_when_completion_is_impossible():
    assert (
        P.completion_witness(
            {"PG": None, "SG": None},
            ["shaquille-o-neal"],
            career_rights("shaquille-o-neal"),
        )
        is None
    )
...
def test_can_complete_roster_ignores_players_already_on_the_roster():
    ...
    for slot in ("PG", "SG", "SF", "PF", "bench_1"):
        roster[slot] = None
    assert not P.can_complete_roster(
        roster, ["shaquille-o-neal"], career_rights("shaquille-o-neal")
    )
