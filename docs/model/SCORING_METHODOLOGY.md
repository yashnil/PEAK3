# PEAK3 scoring methodology and CLI reference

This is the full product + methodology reference for the `peak3` model: what
each component measures, how the official weights were chosen, how missing data
and eras are handled, and the complete CLI.

It was previously the body of the repository README. It moved here when the
README became the PEAK3 Arena product overview; the text below is unchanged.

- For the deepest derivation of individual metrics, see [`../../METHODOLOGY.md`](../../METHODOLOGY.md).
- For a regenerated 25-player validation run, see [`../../outputs.txt`](../../outputs.txt).
