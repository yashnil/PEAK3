...
| EXTRA-18 | `components/layout/Footer.tsx` | **Found by the full E2E suite (1 failure in 408).** The site footer's nav links used a raw Tailwind `text-[var(--text-muted)]`, and axe flagged them `serious: color-contrast` — intermittently, because it only fails once a modal scrim composites over them. They are navigation, not helper text. | **FIXED** | Promoted to `--text-secondary` (the tier SHARED-05 widened precisely so this distinction could carry meaning). 8/8 clean runs of the previously 1-in-5 flaky test. |
| EXTRA-10 | `lib/twenty-dollar-seen.ts:97` | **Found by the adversarial pass and reproduced.** Storage is not all-or-nothing: reads can succeed while writes throw (quota, Safari private browsing, a partitioned frame). `readSeenCursor` returned the persisted value and never consulted the in-memory fallback, so every acknowledgement landed in memory unread — `acknowledgeRecap` was inert for the life of the tab and the recap came straight back. Exactly §16's "caught up banner reappearing after acknowledgment". | **FIXED** | `Math.max(persisted, memory)`. Regression test drives `setItem` throwing while `getItem` works. |
| EXTRA-11 | `$20 Showdown` header vs lot panel | **Found by the adversarial pass.** Two different totals for the same auction on one screen: the header chip read `STANDARD MARKET · 24 LOTS` while the lot panel read `LOT 1 OF 36`. `max_lots` is the hard ceiling; `standard_market_lots` is the market a player is actually in. | **FIXED** | The panel now prints the market's own length and switches to the cap only in closeout. |
| EXTRA-12 | `nba_peak/three_man_weave/positions.py:193` | **Found by the adversarial pass.** `SlotRights` values were mutable `set`s, not the declared `frozenset[str]` — set intersection returns a `set`, and it propagated out through `season_slot_rights` and `card_slot_rights`. Immutability is one of this module's stated safety properties. | **FIXED** | Wrapped in `frozenset`; asserted. |
| EXTRA-13 | `PickOverlay.tsx:385` | **Found by the adversarial pass.** The "Clear" filter chip was the one control in the panel that did not lock at 0 seconds. It submits nothing, so no outcome could change — but a panel that says it is locked while a button still responds says two things at once. | **FIXED** | `disabled={expired}`. |
| EXTRA-14 | `feasibility.py` check 5 | **Found by the adversarial pass; NOT reproduced.** Check 5 asks "can all three rosters still finish?" using `potential_slot_rights`, the union across every card an identity holds. 53 of 1,379 identities (3.8%) have a union strictly wider than any single one of their cards, so in principle the check can promise a completion no actual roll can deliver — and the failure mode is bad: `_open_round` raises, `mode.py` turns it into `REJECT_NO_FEASIBLE_ROLL`, and the player's already-legal pick is refused. | OPEN — documented | Not reproduced in 40 all-timeout matches (720 picks) or the repo's 120 bot matches. Narrowing the check to per-card rights would reject matches that are genuinely playable, so it is recorded rather than changed blind. |
| EXTRA-15 | `PickOverlay` candidate rows | **Found by the adversarial pass.** A row reads "Armen Gilliam · Brooklyn Nets · 1993-94–1995-96 · PF / SF": the positions come from one card season, the span is the whole eligibility range, and that season is never named on an undrafted candidate. Not a legality bug — reducer and label share the card grain — but it is the display half of the grain confusion TMW-02 exists to close. | OPEN | Deferred; recorded rather than rushed at the end of the pass. |
| EXTRA-16 | `LotLedger.tsx:311` | **Found by the adversarial pass; suspected, not reproduced.** `resumeCeiling` seeds from the history max at mount and the cursor is written only after the reveal hold, so a reload *inside* a multi-lot reveal sequence could surface lots the player was present for as a recap. | OPEN | Narrow window (≈4.8s for a three-lot chain). Recorded. |
| EXTRA-17 | `lib/three-man-weave-state.ts:103` | `turnOrder` became dead after `DraftOrderStrip` was deleted — it is now referenced only by its own tests. | OPEN | Left in place rather than churned at the end of the pass; noted so it is removed deliberately rather than rediscovered. |
...
| Gate | Status |
|---|---|
| A — Three-Man Weave gameplay | **MET, with two stated exceptions.** "Player headshots present" is met with designed initials medallions, not photographs — the asset source resolves ~0% of historical players and its licensing gate is off (see the imagery constraint above); that is a licensing decision, not an implementation one. "Spinner is a polished overlay … held long enough to read" is met for the rounds a bot leads and deliberately **not** for the rounds the human leads: the ceremony is gated on there being no open human decision window, which is what removes the timer race. Having both needs a server-side reveal phase. |
| B — $20 Showdown gameplay | **MET, with the same headshot exception.** Reconnect/two-tab safety is proven by unit tests and the state machine, not by a real two-tab browser session. |
| C — Daily Grid | **MET**, headshot exception as above. |
| D — Homepage / facts | **MET.** |
| E — Visual system | **MET**, except that eight `motion/react` files outside the two target games still animate without a reduced-motion guard (EXTRA-01) — deliberately not touched, because fixing them means editing Peak Duel and Run the Table in a pass scoped elsewhere. |
| F — Validation | **MET, except the visual review matrix.** Model 1,539 passed + 1 xfailed · API 1,629 passed + 2 skipped · frontend-verify green (typecheck, lint at 0 warnings, 74 files / 1,879 unit tests, clean production build) · **E2E 408 passed, 0 failed at zero retries** after the one failure it found was fixed. `git diff --check` clean, working tree clean, 5 commits, nothing pushed or deployed. The matrix has 8 of 65 cells uncaptured (both result screens and the reconnect recap) — stated, not quietly dropped. |
