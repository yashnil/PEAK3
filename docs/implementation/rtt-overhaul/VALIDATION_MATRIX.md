...
> called done.
| Playwright full suite, **zero retries** | `scripts/ci/e2e-tests.sh` | never invoked in this pass | **345 passed / 0 failed**, 20.4 min, `PLAYWRIGHT_RETRIES=0`, run completed (not terminated — verified from the log, not the exit code). First real run was 340/5; all five defects fixed: 3 RTT reveal tests encoding pre-ruling skip behaviour, 2 racing client hydration | **PASS** |
