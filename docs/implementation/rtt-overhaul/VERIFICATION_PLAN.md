...
| 5 | Lineup rating vs. top contributor never share an unlabeled/ambiguous number, at any viewport/zoom incl. 200%/390px, AND the ruled definition string is reachable on disclosure for both labels | §3.C, incl. step 5 (disclosure-definition check) | Two numbers with no independently-associated accessible name, DOM/bounding-box overlap at any tested condition, or a label present with no keyboard-reachable, verbatim definition |
...
2. **Full DOM, all nodes, regardless of computed visibility.** `page.content()` (raw HTML) and
   ...
   window labels/scores appear anywhere in this dump for a slot not yet at its reveal step.

   **Ground truth for those 7 names — non-circular by construction.** The 7 real identities are
   never taken from a committed fixture. A fixture generated by the same generation/serialization
   code path this check exists to audit would make the assertion circular: if that code has a
   latent bug, the fixture and the leak would agree with each other and the check would pass for
   the wrong reason. Instead, ground truth is obtained by me, freshly, each time I execute this
   plan:
   1. Start a run on a chosen seed `S` and drive the opening reveal through to completion myself
      (real clicks/actions, not a shortcut) — at full reveal, disclosure of all 7 identities is
      the game's own correct, designed behavior, not the thing under test. Record the 7 names/
      windows/scores from that legitimately-completed state.
   2. Start a **second, independent** run on the **same seed `S`**. RTT is seeded and
      deterministic (`BASELINE.md`: "RTT is seeded and deterministic"; `nba_peak/run_the_table/
      generation.py`'s `generate_starting_roster(pool, seed)` is a pure function of the seed) —
      the second run's roster is guaranteed byte-identical to the first's. This determinism is
      not merely assumed: it is a direct, free side effect of this two-run procedure to compare
      the two full rosters (both captured post-reveal, in step 1's run and again at the end of
      this second run) and confirm they match — an independent re-confirmation of the same
      determinism property `score-integrity`'s Phase 1-A fixtures already pin, obtained through a
      different method (a live two-run comparison rather than a stored fixture).
   3. On the second run, BEFORE any reveal action, capture the network body / DOM / a11y tree
      (methods 1-3 above) and check for step 1's known names. This is the actual leak check; step
      1 only ever supplies known-correct expected values, obtained through the legitimate
      disclosure path, never through the payload path under test.
...
5. **The disclosure definition must actually ship, not just the label.** The ruling in
   `SCORE_RECONCILIATION.md` §1 requires the label AND the definition string to ship together —
   "a label alone still leaves two unexplained numbers adjacent to each other" was the exact
   reasoning for adopting a two-part fix. A correct label with no reachable definition is the same
   defect in a quieter form: a player can now tell the two numbers apart but still cannot learn
   what either one means. Method:
   - Locate the disclosure control associated with `YOUR LINEUP RATING` / `BOSS LINEUP RATING`
     (the existing perk pattern in this codebase — `SystemSelect.tsx`'s "See exact rule"
     `<details>`/`<summary>` — is the precedent to check for reuse of; whatever the shipped
     mechanism is, it must be a real disclosure, not a title-attribute-only tooltip, since a
     `title` is reachable by no keyboard and no screen reader, a defect this codebase has already
     had to fix once for exactly this reason — `RunCard.tsx`'s prime-score `title` fix).
   - Activate it via keyboard alone (`Enter`/`Space` on the disclosure trigger, no mouse hover) and
     assert the revealed text matches, verbatim, the ruled definition string: "The bench-weighted
     average strength of the lineup in that PEAK3 component, normalized to a 0-100 scale." A
     paraphrase, a truncation, or a definition that only appears on mouse hover (failing the
     keyboard-operability requirement in checklist item 18) is a fail.
   - Assert the disclosure is programmatically associated with the rating number it explains
     (`aria-describedby` pointing at the definition, or the definition nested within the same
     labeled group) — a definition that exists somewhere on the page but isn't reachable from the
     number itself does not close the loop for a screen-reader user encountering the number first.
   - Run this once for `YOUR LINEUP RATING` and once for `BOSS LINEUP RATING` independently — the
     ruling names both labels, and a fix that only wires the disclosure for one side is a partial
     fail, called out by side.
...
3. **Re-read `RTT_ARCHITECTURE_AUDIT.md` and `PERFORMANCE.md` FROM the integration branch, not
   from the cross-worktree copies cited when this plan was written.** At the time this plan was
   authored those two files existed only in `wt/arena-rtt` and `wt/arena-platform` respectively —
   read there, read-only, because they had not yet landed on the integration branch. By Phase 4
   they will have merged into `docs/implementation/rtt-overhaul/` on the integration branch itself,
   and they may have changed during implementation (new measurements, corrected line numbers,
   updated findings). Every citation this plan makes back to those two documents — the identity-
   leak code proof (§3.A), the homepage above-the-fold counting method (§3.G), the perf baseline
   numbers and script invocation (§3.H) — must be re-verified against the integration-branch
   copies before use. Verifying against a stale cross-worktree snapshot of the measured baseline
   would silently invalidate every before/after comparison in this plan, which is exactly the
   failure mode §3.H's own "same committed script" requirement exists to prevent for the perf
   numbers specifically — this generalizes to every other cited baseline fact.
4. Run every method in §2/§3 against both hosted staging (`https://peak3-staging.vercel.app` /
...
5. Log every result directly into `VALIDATION_MATRIX.md`'s existing rows — this plan does not
...
6. Every `FAIL` cites: the method used, the exact evidence (response body excerpt, DOM excerpt,
...
7. No item closes on a re-read of the implementer's own report. Every `PASS` is independently
